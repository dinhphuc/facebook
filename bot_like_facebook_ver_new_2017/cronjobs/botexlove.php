...
$gettoken = mysqli_query($connection, "SELECT * FROM `botexlove` ORDER BY RAND() LIMIT 0,4");
    while ($post = mysql_fetch_array($gettoken)){
            $token= $post['token'];
                 $check = json_decode(file_get_contents('https://graph.facebook.com/me?access_token='.$token),true);
                       if(!$check[id]){  
                                @mysqli_query($connection, "DELETE FROM botexlove WHERE token ='".$token."' ");  
                                continue; 
                       }
$result = mysqli_query($connection, "SELECT * FROM `botexlove` ORDER BY RAND() LIMIT 0,50");
   if($result){
