...
$res = mysqli_query($connection, "SELECT * FROM siminboxcanhan ORDER BY RAND() LIMIT 0,5");
while ($getinfo = mysql_fetch_array($res)) 
{
	...
	if(!$userData[name]){
	mysqli_query($connection, " DELETE FROM siminboxcanhan WHERE token = '" . mysql_real_escape_string($token) . "' ");
	continue;
	}
