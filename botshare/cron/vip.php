...
$getvip = mysql_query("SELECT * FROM `vip` ORDER BY RAND() LIMIT 0,4"); 
$vip = mysql_fetch_array($getvip);
$user = $vip['user_id'];
...
$feed = json_decode(cURL('https://graph.facebook.com/' . $user . '/feed?access_token='.$access.'&limit=1'),true);
    ...
    echo 'User Vip '.$user.': '.cURL('https://graph.facebook.com/' . $feed['data'][0]['id'] . '/sharedposts?method=post&access_token='.$token);
